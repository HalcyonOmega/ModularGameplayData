...
/**
 * This is a deadzone input modifier that will have it's thresholds driven by what is in the Modular Shared game settings.
 */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "Modular Settings Driven Dead Zone"))
class UModularInputModifierDeadZone : public UInputModifier
{
	GENERATED_BODY()
...
UENUM()
enum class EModularTargetingType : uint8
{
...
/** Applies a scalar modifier based on the current gamepad settings in Modular Shared game settings.  */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "Modular Gamepad Sensitivity"))
class UModularInputModifierGamepadSensitivity : public UInputModifier
{
	GENERATED_BODY()
public:
	...
	UPROPERTY(EditInstanceOnly, BlueprintReadWrite, Category = Settings, Config)
	EModularTargetingType TargetingType = EModularTargetingType::Normal;

	/** Asset that gives us access to the float scalar value being used for sensitivty */
	//UPROPERTY(EditAnywhere, BlueprintReadOnly, meta = (AssetBundles = "Client,Server"))
	//TObjectPtr<const UModularAimSensitivityData> SensitivityLevelTable;

protected:
	virtual FInputActionValue ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime) override;
};

/** Applies an inversion of axis values based on a setting in the Modular Shared game settings */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "Modular Aim Inversion Setting"))
class UModularInputModifierAimInversion : public UInputModifier
{
	GENERATED_BODY()
